...
export default class Query {
  ...
  orderBy(...args: {[T]: 123}[]): Query
  orderBy <T,U>(...args: {[T]:U:'asc' | 'asc_nulls_first' | 'asc_nulls_last' | 'desc' | 'desc_nulls_first' | 'desc_nulls_last'}): Query {
    if (args.length % 2 != 0) {
  ...
  query() {
    this.orderBy({ field: 'asdas', operator: 'asc' })
    this.orderBy('asda')
    return `query {
      ${this._schema}${ }{

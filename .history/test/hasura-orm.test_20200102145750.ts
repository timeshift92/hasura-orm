...
describe("Query test", () => {
  ...
  it('Query select', () => {
    ...
    expect(new Query('products').select('1,2,3').parsed()).toBeInstanceOf(Query)
    expect(new Query('products').select('1,2,3').where('id','in',[1,2,3]).parsed()).toBeTruthy()
    const query = new Query('products').select('id,rest,price')

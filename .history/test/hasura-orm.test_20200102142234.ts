  ...
  it("Query where error", () => {
    ...
    try {
      new Query('products').where('rest', 'gte', '1', '2')
       ...
       expect(e).toMatch(new Error("where need min: 2 and max: 3 args"));
    }
    expect(new Query('products').where('rest', 'gte', '1', '2')).toThrowError(new Error("where need min: 2 and max: 3 args"))
  })

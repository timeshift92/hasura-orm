...
export default class Hasura {
  ...
  where(...condition: any[]) {
    ...
    if (condition && condition.length === 1) {
      where += stringify(condition[0])
    } else if (!condition || (Array.isArray(condition) && condition.length > 3)) {
  ...
  parsed() {
    return `${this._paginate} ${this._schema} ${
      Object.keys(this._schemaArguments).length > 0 ? '(' + this._schemaArguments + ')' : ''
